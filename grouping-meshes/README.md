...
Now make sure that your **Spotlight** is selected in the **World Outliner**. Then press the <kbd>Blueprint/Add Script</kbd> button to turn this from a actor to a reusable blueprint. Add it to the **Blueprints** folder and call it `BP_Spotlight`. Make sure **New Subclass** is selected then press the Select button. Create a new folder called `Room3` and move the new blueprint into the folder.
