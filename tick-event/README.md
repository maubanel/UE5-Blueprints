...
The **Tick Event** updates every frame which is upwards of 90FPS here, which is why you can't make out what is happening.  It doesn't have to be so.  You can go to the top node **BP_Timer**, in the component and set the **Tick Interval** from `0.0` (no delay) to `1.0` (updates once a second).
